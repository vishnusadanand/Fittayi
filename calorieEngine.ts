...
// Default meal distribution — swap for a user-config lookup later
// if you add intermittent fasting / custom meal counts.
const DEFAULT_MEAL_SPLIT: Record<MealSlot, number> = {
  breakfast: 0.25,
  lunch: 0.35,
  dinner: 0.3,
  snack: 0.1,
...
//   goal: "cut",
// });
// const mealPlan = distributeMeals(result);
//
// If result.wasFloorClamped is true, surface result.clampReason to the user
// in the UI before generating their plan (see Step 4 discussion: transparency
// matters here, don't silently override their selected goal).
